# Quick reference
 ...
 2. Obtain the Meryl docker image (choose one):
 - Pull the pre-built Docker image from DockerHub
   - ```docker pull openeuler/meryl:1.4.1-oe2403sp3```
 - Or build the image locally from source
   - ```docker build -t openeuler/meryl:1.4.1-oe2403sp3 1.4.1/24.03-lts-sp3/```
 3. Run the Docker container to launch the Meryl environment.
 - ```docker run -it openeuler/meryl:1.4.1-oe2403sp3```
 4. Verify the installation inside the container:
 - Check the Meryl version:
   - ```meryl -version```
 - Count k-mers from a FASTA file:
   - ```meryl count k=21 input.fasta output merylDB```
 - View k-mer statistics:
   - ```meryl statistics merylDB```
