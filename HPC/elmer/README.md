...
- Obtain the Elmer docker image from DockerHub:
```docker pull openeuler/elmer:{Tag}```
- Run the Docker container to launch the Elmer environment.
```docker run -it openeuler/elmer:{Tag}```
