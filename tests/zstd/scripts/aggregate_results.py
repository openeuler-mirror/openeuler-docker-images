...
def compute_summary(primary, micro):
    ...
    for name, res in rs.items():
        if not isinstance(res, dict):
            continue
        lvl = res.get("level")
        cs = safe_float(res.get("compress_speed_mbs", 0)) if res.get("compress_speed_mbs") else None
        ds = safe_float(res.get("decompress_speed_mbs", 0)) if res.get("decompress_speed_mbs") else None
        cr = safe_float(res.get("compression_ratio", 0)) if res.get("compression_ratio") else None
        if cs is not None:
            compress_speeds.append(cs)
        if ds is not None:
            decompress_speeds.append(ds)
        if cr is not None:
            ratios.append(cr)
        if lvl is not None:
            if cs is not None:
                per_level_compress.setdefault(str(lvl), []).append(cs)
            if cr is not None:
                per_level_ratio.setdefault(str(lvl), []).append(cr)
    ...
    for lvl_str, vals in sorted(per_level_compress.items()):
        summary[f"level{lvl_str}_avg_compress_speed_mbs"] = round(sum(vals) / len(vals), 2)
    for lvl_str, vals in sorted(per_level_ratio.items()):
        summary[f"level{lvl_str}_avg_compression_ratio"] = round(sum(vals) / len(vals), 4)
...
def aggregate_results(results_dir, output_file):
    ...
    result = {
        ...
        "summary": summary,
        "software": "zstd",
        "version": version_info.get("software_version", "1.5.6"),
    }
