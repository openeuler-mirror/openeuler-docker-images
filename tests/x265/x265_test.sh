...
create_build_tmpdir() { BUILD_TMPDIR="$(mktemp -d /tmp/x265_build_XXXXXX)"; log "BUILD" "Created temp build dir: ${BUILD_TMPDIR}"; }
...
check_prerequisites() {
    ...
    { command -v g++ >/dev/null 2>&1 || command -v c++ >/dev/null 2>&1; } && log "CHECK" "G++ OK: $(g++ --version 2>&1 | head -1)" || log "WARN" "g++ not found - will install"
    ...
    log "CHECK" "Build method: ${BUILD_METHOD} (cmake, NEON auto-detected on aarch64)"
    return ${err}
}

phase1_build() {
    log "PHASE1" "=== Phase 1: Source Build x265 v${SOFTWARE_VERSION} ==="
    create_build_tmpdir
    local SRC="${BUILD_TMPDIR}/x265_src"
    ...
    case "${os_id_lower}" in
        ubuntu|debian) sudo apt-get update -qq 2>&1 | tee -a "${LOG_FILE}" >/dev/null; sudo apt-get install -y -qq build-essential g++ cmake make git wget curl 2>&1 | tee -a "${LOG_FILE}" >/dev/null ;;
        openeuler) sudo dnf install -y gcc gcc-c++ cmake make git wget curl 2>&1 | tee -a "${LOG_FILE}" >/dev/null ;;
        centos|rhel|fedora) sudo dnf install -y gcc gcc-c++ cmake make git wget curl 2>&1 | tee -a "${LOG_FILE}" >/dev/null ;;
        *) log "WARN" "Unknown OS: ${os_id}, generic build..." ;;
    esac

    log "PHASE1" "Cloning x265 tag ${SOFTWARE_VERSION} (from Bitbucket)..."
    git clone --branch "${SOFTWARE_VERSION}" --depth 1 https://bitbucket.org/multicoreware/x265_git.git "${SRC}" 2>&1 | tee -a "${LOG_FILE}" || {
        log "WARN" "shallow clone failed, trying full clone + checkout..."
        git clone https://bitbucket.org/multicoreware/x265_git.git "${SRC}" 2>&1 | tee -a "${LOG_FILE}" \
            && (cd "${SRC}" && git checkout "${SOFTWARE_VERSION}" 2>&1 | tee -a "${LOG_FILE}") || { log "ERROR" "Failed to clone x265 tag ${SOFTWARE_VERSION}"; return 1; }
    }

    log "PHASE1" "Configuring CMake..."
    mkdir -p "${BUILD}"
    (cd "${BUILD}" && cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX="${INSTALL}" \
        -DENABLE_CLI=ON -DENABLE_SHARED=OFF -DENABLE_HDR=OFF "${SRC}/source" 2>&1 | tee -a "${LOG_FILE}") || { log "ERROR" "cmake failed"; return 1; }

    log "PHASE1" "Compiling x265..."
    (cd "${BUILD}" && make -j$(nproc) 2>&1 | tee -a "${LOG_FILE}") || { log "ERROR" "make failed"; return 1; }
    (cd "${BUILD}" && make install 2>&1 | tee -a "${LOG_FILE}") || log "WARN" "make install failed, using in-tree binary"

    BENCHMARK_BIN="${INSTALL}/bin/x265"
    if [ ! -x "${BENCHMARK_BIN}" ]; then
        if [ -x "${BUILD}/x265" ]; then BENCHMARK_BIN="${BUILD}/x265"; else log "ERROR" "x265 binary not found"; return 1; fi
    fi

    log "PHASE1" "Verifying x265 binary..."
    "${BENCHMARK_BIN}" -V 2>&1 | tee -a "${LOG_FILE}" | head -1 || log "WARN" "version check failed"
...
phase2_verify() {
    ...
    local timestamp model arch kernel os_name cpu_model cores python_ver gcc_ver x265_ver
    ...
    x265_ver="${SOFTWARE_VERSION}"
    if [ -x "${BENCHMARK_BIN}" ]; then
        x265_ver="$("${BENCHMARK_BIN}" -V 2>&1 | head -1 | tr -d '\n\t' || echo "${SOFTWARE_VERSION}")"
    fi
    python3 "${JSON_HELPER}" "${RESULTS_DIR}/version_info.json" write_version_info \
        "${timestamp}" "${model}" "${arch}" "${kernel}" "${os_name}" "${cpu_model}" \
        "${cores}" "${SOFTWARE_NAME}" "${x265_ver}" \
        "${python_ver}" "${gcc_ver}"
    log "PHASE2" "Version info saved (x265: ${x265_ver})"
}

phase3_run_benchmarks() {
    log "PHASE3" "=== Phase 3: Run Benchmarks ==="
    mkdir -p "${RESULTS_DIR}"
    log "PHASE3A" "Running H.265 encode preset sweep..."
...
testSoftwareIsInstalled() { local f=0; [ -n "${BENCHMARK_BIN}" ] && [ -x "${BENCHMARK_BIN}" ] && f=1; if [ "${f}" -eq 0 ]; then echo "WARNING: x265 binary not found"; startSkipping; return; fi; assertTrue "x265 binary should be executable" "[ ${f} -eq 1 ]"; }
...
x265 Source Build & Performance Benchmark (shUnit2)
Options:
  --check    Check prerequisites only
  -h|--help  Show this help
Environment variables:
  SOFTWARE_VERSION   x265 version tag (default: 4.2; cloned from Bitbucket)
  ...
  MIN_FPS            Minimum medium-preset fps threshold (default: 2; HEVC is ~10x slower than H.264)
Examples:
  ./x265_test.sh --check
  ./x265_test.sh
  SOFTWARE_VERSION=4.2 YUV_FRAMES=100 ITERATIONS=3 ./x265_test.sh
USAGE
}
