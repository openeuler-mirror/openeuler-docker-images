...
def generate_summary(input_json, output_file):
    ...
    lines = []
    lines.append("=" * 70)
    lines.append("  x265 Source Build & Performance Benchmark Report")
    lines.append("=" * 70)
    ...
    if env:
        ...
        lines.append(f"  x265 Version:     {env.get('software_version', 'N/A')}")
    ...
    if primary:
        lines.append("  --- H.265 Encode Benchmark (Primary, preset sweep) ---")
    ...
    lines.append("=" * 70)
    lines.append("  Report generated by x265 Source Build & Performance Benchmark Workflow")
    lines.append("=" * 70)
