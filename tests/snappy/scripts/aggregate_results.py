...
def compute_summary(primary, micro):
    ...
    if isinstance(mresults, dict):
        ...
        if isinstance(mt, dict):
            one = safe_float(mt.get("threads_1", {}).get("avg_compress_speed_mbs", 0)) if isinstance(mt.get("threads_1"), dict) else 0
            allq = safe_float(mt.get("threads_all", {}).get("avg_compress_speed_mbs", 0)) if isinstance(mt.get("threads_all"), dict) else 0
            if one > 0 and allq > 0:
...
def aggregate_results(results_dir, output_file):
    ...
    result = {
        ...
        "summary": summary,
        "software": "snappy",
        "version": version_info.get("software_version", "1.2.2"),
    }
