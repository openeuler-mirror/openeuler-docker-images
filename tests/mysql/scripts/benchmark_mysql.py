#!/usr/bin/env python3
"""MySQL OLTP benchmark via sysbench — same pattern as OceanBase."""
import subprocess
import re
import sys
import os
...
def sysbench_prepare(host, port, user, password, db_name, tables, table_size):
    ...
    print(f"[BENCHMARK_MYSQL] Preparing: {tables} tables x {table_size} rows...")
    result = subprocess.run(cmd, capture_output=True, text=True, timeout=600)
    if result.returncode != 0:
        print(f"[BENCHMARK_MYSQL] prepare failed: {result.stderr[:500]}")
        return False
    print("[BENCHMARK_MYSQL] Data prepared.")
    return True
...
def sysbench_run(host, port, user, password, db_name, workload, threads, tables, table_size, time_sec=60):
    ...
    if tps == 0 and qps == 0:
        print(f"[BENCHMARK_MYSQL][DEBUG] {workload} t={threads} parse failed. Raw (last 1500):")
        print(text[-1500:])
    return {"tps": round(tps, 2), "qps": round(qps, 2), "avg_latency_ms": round(avg_lat, 2), "p95_latency_ms": round(p95, 2)}


def main():
    if len(sys.argv) < 9:
        print("Usage: benchmark_mysql.py <output_file> <host> <port> <user> <password> <db_name> <tables> <table_size> [iterations] [time_sec]")
        sys.exit(1)
    output_file = sys.argv[1]
    host, port, user, password = sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5]
    db_name, tables, table_size = sys.argv[6], int(sys.argv[7]), int(sys.argv[8])
    iterations = int(sys.argv[9]) if len(sys.argv) >= 10 else 1
    time_sec = int(sys.argv[10]) if len(sys.argv) >= 11 else 60
    version_str = os.environ.get("SOFTWARE_VERSION", "8.0")

    print(f"[BENCHMARK_MYSQL] Checking MySQL at {host}:{port}...")
    if not check_mysql(host, port, user, password):
        print(f"[BENCHMARK_MYSQL] Cannot connect to MySQL at {host}:{port}")
        sys.exit(1)
    print(f"[BENCHMARK_MYSQL] Creating database {db_name}...")
    if not create_database(host, port, user, password, db_name):
        print("[BENCHMARK_MYSQL] Failed to create database")
        sys.exit(1)
    if not sysbench_prepare(host, port, user, password, db_name, tables, table_size):
        print("[BENCHMARK_MYSQL] sysbench prepare failed")
        sys.exit(1)
    ...
    for wl in WORKLOADS:
        ...
        for threads in THREAD_LEVELS:
            ...
            print(f"[BENCHMARK_MYSQL] {wl} @ threads={threads}...")
            runs = []
    ...
    out = {
        "benchmark": "oltp",
        "description": f"MySQL OLTP benchmark via sysbench ({tables} tables x {table_size} rows, {time_sec}s per test) on ARM64",
        "reference": "https://github.com/mysql/mysql-server",
        "software": "mysql",
        "version": version_str,
        "architecture": "arm64",
        ...
        "performance_metrics": {
            ...
            "avg_latency_ms": {"unit": "ms", "description": "Average latency"},
            "p95_latency_ms": {"unit": "ms", "description": "95th percentile latency"},
        },
        "parameters": {
            "workloads": WORKLOADS, "thread_levels": THREAD_LEVELS,
            "tables": tables, "table_size": table_size,
            "time_per_test": time_sec, "iterations": iterations,
    ...
    print(f"[BENCHMARK_MYSQL] Output written to {output_file} ({len(results_summary)} workloads)")
