...
def generate_summary(input_json, output_file):
    ...
    lines = []
    lines.append("=" * 70)
    lines.append("  Faiss Source Build & Performance Benchmark Report")
    lines.append("=" * 70)
    ...
    build_method = env.get('build_method', 'source_build') if env else 'source_build'
    if env:
        ...
        lines.append(f"  Build OS:       {env.get('build_os', env.get('os', 'N/A'))}")
        lines.append(f"  Build Method:   {build_method}")
        lines.append("")

    ann = data.get("benchmarks", {}).get("primary", data.get("primary_benchmark", {}))
    if ann:
    ...
    if summary:
        ...
        if "avg_qps" in summary:
            lines.append(f"    Avg QPS:              {summary['avg_qps']} queries/sec")
        if "max_qps" in summary:
            lines.append(f"    Max QPS:              {summary['max_qps']} queries/sec")
        if "avg_recall_at_k" in summary:
            lines.append(f"    Avg Recall@k:         {summary['avg_recall_at_k']}")
        if "avg_latency_us" in summary:
            lines.append(f"    Avg Latency:          {summary['avg_latency_us']} us")
        if "avg_build_time_s" in summary:
            lines.append(f"    Avg Build time:       {summary['avg_build_time_s']}s")
        if "add_rate_per_sec" in summary:
            lines.append(f"    Add rate:             {summary['add_rate_per_sec']} vec/s")
        if "multithread_scaling_ratio" in summary:
            lines.append(f"    Multithread scaling:  {summary['multithread_scaling_ratio']}x")
        lines.append("")

    lines.append("=" * 70)
    lines.append("  Report generated by Faiss Source Build & Performance Benchmark Workflow")
    lines.append("=" * 70)
