...
static BenchResult run_compression_bench(const std::string& input, int iterations) {
    ...
    int dst_cap = LZ4_compressBound(src_size);
    std::string compressed(dst_cap, '\0');
    int comp_size = LZ4_compress_default(input.data(), &compressed[0], src_size, dst_cap);
    compressed.resize(comp_size > 0 ? comp_size : 0);
    r.compressed_size = compressed.size();
    r.compression_ratio = (comp_size > 0)
        ? static_cast<double>(input.size()) / static_cast<double>(comp_size) : 0.0;
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        int cs = LZ4_compress_default(input.data(), &comp_out[0], src_size, dst_cap);
        ...
        LZ4_decompress_safe(comp_out.data(), &decomp_out[0], cs, src_size);
...
static MicroBlockResult run_block_bench(size_t block_size, int iterations) {
    ...
    int src_size = static_cast<int>(block_size);
    int dst_cap = LZ4_compressBound(src_size);
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        if (i == 0 && cs > 0) compressed_size = cs;
        if (cs <= 0) continue;
...
static MtResult run_mt_bench(size_t block_size, int num_threads, int iterations) {
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        std::vector<int> comp_sizes(num_threads, 0);

        auto c_start = std::chrono::high_resolution_clock::now();
        for (int t = 0; t < num_threads; t++) {
            threads.emplace_back([&, t]() {
                comp_sizes[t] = LZ4_compress_default(input.data(), &comp_results[t][0], src_size, dst_cap);
            });
        }
        ...
        for (int t = 0; t < num_threads; t++) {
            d_threads.emplace_back([&, t]() {
                if (comp_sizes[t] > 0)
                    LZ4_decompress_safe(comp_results[t].data(), &decomp_results[t][0], comp_sizes[t], src_size);
            });
        }
...
int main(int argc, char* argv[]) {
    ...
    std::string version_str = env_version ? env_version : "1.9.4";
