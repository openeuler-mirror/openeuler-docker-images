...
def generate_summary(input_json, output_file):
    ...
    lines = []
    lines.append("=" * 70)
    lines.append("  rust Performance Benchmark Report")
    lines.append("=" * 70)
    ...
    if env:
        ...
        lines.append(f"  rust Version:   {env.get('software_version', 'N/A')}")
    ...
    primary = data.get("benchmarks", {}).get("primary", data.get("primary_benchmark", {}))
    if primary:
    ...
    if summary:
        ...
        if "compile_hello_avg_s" in summary:
            lines.append(f"    Compile hello avg:   {summary['compile_hello_avg_s']}s")
        if "compile_matmul_avg_s" in summary:
            lines.append(f"    Compile matmul avg:  {summary['compile_matmul_avg_s']}s")
        if "matmul_500x500_avg_s" in summary:
            lines.append(f"    Matmul 500x500 avg:  {summary['matmul_500x500_avg_s']}s")
        if "operation_count" in summary:
            lines.append(f"    Operation count:     {summary['operation_count']}")
        if "avg_time_s" in summary:
            lines.append(f"    Avg operation time:   {summary['avg_time_s']}s")
        lines.append("")

    lines.append("=" * 70)
    lines.append("  Report generated by rust Performance Benchmark Workflow")
    lines.append("=" * 70)
