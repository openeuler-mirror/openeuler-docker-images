...
def main():
    ...
    if success and os.path.exists(output_file):
        try:
            ...
            if "parse_latency_by_size" in results:
                print(f"[BENCHMARK_MICRO] Doc sizes: {list(results['parse_latency_by_size'].keys())}")
            if "multithread_parse" in results:
                print(f"[BENCHMARK_MICRO] Thread counts: {list(results['multithread_parse'].keys())}")
