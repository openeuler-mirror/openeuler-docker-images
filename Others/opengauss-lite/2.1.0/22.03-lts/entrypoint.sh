...
# used to create initial opengauss directories and if run as root, ensure ownership belong to the opengauss user
docker_create_db_directories() {
        ...
        if [ -n "$POSTGRES_INITDB_XLOGDIR" ]; then
                ...
                if [ "$user" = '0' ]; then
                        find "$POSTGRES_INITDB_XLOGDIR" \! -user opengauss -exec chown opengauss '{}' +
                fi
        ...
        if [ "$user" = '0' ]; then
                find "$PGDATA" \! -user opengauss -exec chown opengauss '{}' +
                find /var/run/opengauss \! -user opengauss -exec chown opengauss '{}' +
        fi
}
...
docker_setup_env() {
        export GS_USER=opengauss
        file_env 'GS_PASSWORD'

        # file_env 'GS_USER' 'opengauss'
...
_main() {
        ...
        if [ "$1" = 'gaussdb' ] && ! _opengauss_want_help "$@"; then
                ...
                if [ "$(id -u)" = '0' ]; then
                        ...
                        exec gosu opengauss "$BASH_SOURCE" "$@"
                fi
